...
from conf_single_pendulum import *
from ocp_single_pendulum import *
import re
...
def animate_all_simulations_together(file_paths, key="q_trajectory"):
    ...
    for i, file_path in enumerate(file_paths):
        try:
            ...
            if key in data:
                ...
                line, = ax.plot([], [], 'o-', lw=2)
                traj_line, = ax.plot([], [], 'r--', lw=1, label='Trajectory Link 1')
    ...
    def init():
        for line, traj_line in zip(lines, trajectory_lines):
            line.set_data([], [])
            traj_line.set_data([], [])
...
if __name__ == "__main__":
    ...
    filename_ocp = 'dataset/ocp_dataset_SP_train.csv'
