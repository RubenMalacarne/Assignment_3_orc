# -*- coding: utf-8 -*-
"""
Configuration file for Triple Pendulum
"""
import numpy as np
...
q_des = np.array([0,0,0])# Default: "pendolo a piombo" --> np.array([-np.pi, -np.pi*2])
q0 = np.array([np.pi, np.pi, np.pi])
random_initial_set = True
n_init_state_ocp = 1100 #100 for eval # Avoid odd numbers if disliked

# STEP VALUE  ------------------------------------------------------------------------------------
dt_sim = 0.002  
dt = 0.01       # OCP time step
N_sim = 1000     # Simulation steps for MPC

N_step = 100     # Time horizon N steps for OCP
M_step = 50     # Time Horizon M steps for OCP
...
# FILE PATH ----------------------------------------------------------------------------------------
csv_train = 'dataset/ocp_dataset_TP_train.csv'
csv_eval ='dataset/ocp_dataset_TP_eval.csv'
